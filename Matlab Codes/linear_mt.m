% Linear Model (memristance v/s time)
clc;
clear ;
close all;
...
r_i = 500; % equivalent instantaneous resistance
...
yyaxis left
plot(t,m/1000,'LineWidth',2.0);
ylim([0 20]);
ylabel('Memristance (kohms)');
xlabel('Time(s)');

yyaxis right
plot(t,x,'LineWidth',2.0);
% ylim([0 1.6]);
ylabel('Normalized width (x)');
legend('Memristance', 'Normalized width');
